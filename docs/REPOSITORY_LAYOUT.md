...
The default `main` branch is the Block Engine source repository. The public
website is maintained on the independent `gh-pages` branch and GitHub Pages
publishes that branch's root. This separation keeps the engine's language
statistics representative while preserving the existing public URLs.
...
| `bin/`, `release/` | Local build and release staging directories; ignored and never committed. |
| GitHub Releases | Versioned engine binaries, installer, VSIX, Acode package, and `SHA256SUMS.txt`. The tag workflow generates these from source. |
...
Historical release assets remain available in their GitHub Releases. The main
branch intentionally contains no generated binaries, installers, extension
archives, or stale checksums. Website links target immutable versioned GitHub
Release assets.

The `gh-pages` branch owns the HTML, CSS, JavaScript, images, search metadata,
machine-readable security policy, static-host configuration, and website-only
